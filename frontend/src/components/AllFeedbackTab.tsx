...
import type { Annotations, FeedbackEntry, SessionData, TriageStatus } from '../types'
...
function InlineAnnotation({
  ...
  return (
    <input
      ...
      className="w-full bg-transparent text-xs text-gray-400 placeholder-gray-700
                 border-b border-transparent hover:border-gray-700 focus:border-indigo-500
...
export default function AllFeedbackTab() {
  ...
  // Build session_id -> org_name lookup from cached sessions
  const orgBySession = useMemo(() => {
    const map: Record<string, string> = {}
    sessions?.forEach((s) => { if (s.org_name) map[s.session_id] = s.org_name })
    return map
  }, [sessions])

  const allOrgs = useMemo(() => {
    const orgs = new Set<string>()
    sessions?.forEach((s) => { if (s.org_name) orgs.add(s.org_name) })
  ...
  const filtered = useMemo(() => {
    ...
    return (feedbackData ?? []).filter((e) => {
      ...
      if (orgFilter && orgBySession[e.session_id] !== orgFilter) return false
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Filter bar */}
      <div className="flex-none border-b border-gray-800 bg-gray-950 px-4 py-2.5 space-y-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="text-[11px] px-2.5 py-1 bg-gray-800 border border-gray-700 rounded
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5">
                          <button
                            ...
                            className="font-mono text-indigo-400 hover:text-indigo-300 transition-colors truncate max-w-[120px] block"
                          >
                            {entry.session_id.slice(0, 14)}…
                          </button>
                        </td>
                        <td className="px-3 py-2.5 text-indigo-300/80 text-[11px] font-medium truncate max-w-[120px]">
                          {orgBySession[entry.session_id] ?? <span className="text-gray-700">—</span>}
                        </td>
                        <td className="px-3 py-2.5 text-gray-500 font-mono whitespace-nowrap">
                          {formatTs(entry.timestamp)}
                        </td>
                        <td className="px-3 py-2.5 text-gray-400">{entry.speaker ?? '—'}</td>
                        <td className="px-3 py-2.5">
                          {entry.category ? (
                            <span className="font-mono text-orange-300 bg-orange-900/30 border border-orange-800/40 px-1.5 py-0.5 rounded text-[10px]">
                              {entry.category}
                            </span>
                          ) : (
                        ...
                        <td className="px-3 py-2.5 text-gray-300 max-w-[200px]">
